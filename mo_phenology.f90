...
MODULE mo_phenology
  ...
  USE mo_kind,        ONLY: dp 
  USE mo_jsbach,      ONLY: debug, new_day, new_year
...
CONTAINS
  ...
  SUBROUTINE update_phenology(kidx,pheno_type_of_tile,lai,laiMax_stat,air_temp,relative_moisture,NPP_rate,specLeafArea,lat,relative_wilting_point,leaf_shedding_rate) ! HW
    ...
    REAL(dp),INTENT(in)           :: laiMax_stat(:,:)          !! Prescribed (=static) maximum LAI for the current year. JN: could now theoretically also change within the year
    ...
    !-----> JN
    !before, laiMax_dyn was only set to laiMax_stat if smaller than zero, i.e. only if initialised in init_phenology; i.e. in init_jsbach, i.e. on start/restart!
    !IF (ANY(laiMax_dyn(kidx0:kidx1,:) < 0._dp)) laiMax_dyn(kidx0:kidx1,:) = laiMax_stat(1:kidx,:)

    !Now we set it for each call - Alternatively could be set each new year
!    IF (new_year) THEN
        laiMax_dyn(kidx0:kidx1,:) = laiMax_stat(1:kidx,:)
!    ENDIF
    !<-----JN
